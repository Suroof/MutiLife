<template>
	<view class="playlist-detail-container">
		<!-- 顶部信息区域 -->
		<view class="header-area" :style="{ backgroundImage: `url(${playlist.cover})` }">
			...
			<view class="playlist-info">
				<image class="playlist-cover" :src="playlist.cover || defaultCover" mode="aspectFill"></image>
				<view class="playlist-data">
					<text class="playlist-name">{{playlist.name || '歌单详情'}}</text>
					<view class="creator-info">
						<image class="creator-avatar" :src="playlist.creator && playlist.creator.avatar" mode="aspectFill"></image>
						<text class="creator-name">{{playlist.creator && playlist.creator.nickname || '未知用户'}}</text>
					</view>
					<view class="playlist-stats">
						<text class="stat-item">{{playlist.songCount || 0}}首</text>
						<text class="stat-item">播放{{formatPlayCount(playlist.playCount || 0)}}</text>
					</view>
					<view class="playlist-tags" v-if="playlist.tags && playlist.tags.length">
						<text class="tag-item" v-for="(tag, index) in playlist.tags" :key="index">#{{tag}}</text>
					</view>
				</view>
			</view>
		...
		<view class="song-list-container">
			...
			<view class="list-header">
				<view class="play-all-button" @tap="playAll">
					...
					<text class="song-count">({{playlist.songs ? playlist.songs.length : 0}})</text>
				</view>
			...
			<view class="song-list">
				<view class="song-item" v-for="(song, index) in playlist.songs" :key="song._id" @tap="playSong(song)">
					...
					<view class="song-content">
						<view class="song-main-info">
							...
							<view class="song-badges" v-if="song.badge">
								<text class="song-badge" v-for="(badge, i) in song.badge" :key="i">{{badge}}</text>
							</view>
						</view>
						<view class="song-sub-info">
							<text class="song-artist">{{song.artist}}</text>
							<text class="song-album" v-if="song.album">- {{song.album}}</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async loadPlaylistDetail() {
				...
				try {
					const response = await request.get(`/music/playlists/${this.playlistId}`, {}, true);
			...
			async playSong(song) {
				...
				const currentIndex = this.playlist.songs.findIndex(item => item._id === song._id);
				uni.$emit('setPlayList', {
					playList: this.playlist.songs,
