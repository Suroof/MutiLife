<template>
  <view class="container">
    ...
    <view class="category-tabs">
      ...
      >
        <image class="selectBtn" :src="item.icon" mode="aspectFit">{{
          item.image
        }}</image>
    ...
    <view class="section playlist-section">
      ...
      <view class="playlist-grid">
        <view
          class="playlist-card"
          v-for="(item, index) in hotPlaylists"
          :key="index"
        ...
        >
          ...
          <text class="playlist-desc"
            >{{ item.count || item.songCount || 0 }}首</text
          >
        </view>
      </view>
    </view>
    ...
    <view class="section artists-section">
      ...
      >
        <view
          class="artist-card"
          v-for="(item, index) in recommendedArtists"
          :key="index"
          @tap="openArtist(item)"
        >
          <image
    ...
    <view class="player-bar" v-if="currentSong" @tap="openFullPlayer">
      ...
      <view class="player-controls">
        <text class="iconfont control-icon" @tap.stop="togglePlayStatus">{{
          isPlaying ? "⏸" : "⏵"
        }}</text>
        <text class="iconfont control-icon" @tap.stop="nextSong">⏭</text>
        <view class="volume-control" @tap.stop>
          <text class="iconfont volume-icon" @tap.stop="toggleMute">{{
            volume > 0 ? "🔊" : "🔇"
          }}</text>
          <slider
            class="volume-slider"
    ...
    <view
      class="mini-player"
      v-if="currentSong && currentSong._id && !showFullPlayer"
    >
      <view class="mini-player-left" @tap="openFullPlayer">
        ...
        <view class="mini-song-info">
          <text class="mini-song-name">{{
            currentSong.name || "未知歌曲"
          }}</text>
          <text class="mini-song-artist">{{
      ...
      <view class="mini-player-right">
        <text
          class="mini-player-btn play-pause-btn iconfont"
          @tap.stop="togglePlayStatus"
        >
          {{ isPlaying ? "⏸" : "▶" }}
        </text>
        <text class="mini-player-btn next-btn iconfont" @tap.stop="nextSong"
          >⏭</text
        >
      </view>
    </view>
        ...
        <view class="content-area">
          ...
          <view class="controls-container glass-card">
            ...
            <view class="progress-bar">
              <text class="time-text current-time">{{
              ...
              <text class="time-text total-time">{{
            ...
            <view class="volume-control-full">
              <text class="iconfont volume-icon" @tap="toggleMute">{{
                volume > 0 ? "🔊" : "🔇"
              }}</text>
              <slider
...
export default {
    ...
    return {
      ...
      categories: [
        ...
        },
        {
          ...
          url: "/pages/music/my-list/mylist",
        },
        {
          title: "电台",
  ...
  methods: {
    selectCategory(index) {
     console.log('=== selectCategory 调试信息 ===');
    console.log('1. 方法被调用, index:', index);
    console.log('2. categories是否存在:', !!this.categories);
    console.log('3. categories长度:', this.categories?.length);
    console.log('4. 当前分类数据:', this.categories[index]);
    ...
    nextSong() {
      ...
      // 查找当前歌曲在推荐列表中的位置
      const currentIndex = this.dailyRecommendations.findIndex(
        (song) =>
      ...
      if (
        currentIndex !== -1 &&
        currentIndex < this.dailyRecommendations.length - 1
      ) {
        // 播放下一首歌曲
